...
| **TV Cold Reboot / Power-cut Daemon Disconnected (Standby vs Cold Boot)** | TV reboots and kills all ADB interactive shell background sessions (`nohup`). In daily standby (remote power off), daemon stays alive 24/7. After hard power cut, daemon needs restart. | Check `ps -ef \| grep mihomo` or test port 7890. | 1. **TV-Native (Zero PC)**: Open `YouTube-Proxy-Launcher.apk` on TV to wake proxy in 0.05s and launch SmartTube.<br>2. **PC 1-Click**: Double click `🚀 一键唤醒电视代理(Mihomo).bat` on desktop (0.5s auto-wake). |
| **SELinux Blocks Third-Party TV Apps from Executing `/data/local/tmp/mihomo`** | Android 9+ SELinux policy (`untrusted_app`) restricts standard APKs from executing arbitrary binaries outside their package namespace. | `avc: denied { execute }` in dmesg/logcat when third-party app calls `Runtime.exec`. | Package `mihomo` as a native library (`lib/arm64-v8a/libmihomo.so`) inside `YouTube-Proxy-Launcher.apk`. Android extracts it to `/data/app/<pkg>/lib/` with full execute permissions. |
| **Wi-Fi Takes Long to Connect & Shows "No Internet" on Boot** | 1. Android Captive Portal Check attempts `gstatic.com/generate_204` (blocked in China), hanging for 30~60s.<br>2. Android Global HTTP Proxy is set to `127.0.0.1:7890`, stalling TV system boot before Mihomo is up. | Run `settings get global captive_portal_http_url` and `settings get global http_proxy`. | 1. Clear global proxy: `settings put global http_proxy :0` and delete keys.<br>2. Replace captive portal: `settings put global captive_portal_http_url http://connect.rom.miui.com/generate_204`<br>3. Set NTP: `settings put global ntp_server ntp.aliyun.com`. |
| **Domestic TV Apps (iQIYI / Tencent / Mango) Slowed Down or Proxy Contaminated** | Android Global HTTP Proxy routes all TV traffic through Mihomo, wasting node data and adding latency. | Inspect global proxy vs SmartTube preference. | Remove Android Global Proxy. SmartTube isolates proxy traffic internally via `web_proxy_uri=http://127.0.0.1:7890`. Domestic apps connect directly to local broadband at 0ms latency. |
| **OEM Background App Killers (Skyworth `SkyRMService`) Kill VPN / Accessibility Apps** | OEM proprietary daemons scan and terminate Java/ART background applications (curAdj = 200). Child processes started via `Runtime.exec` are killed due to Cgroup inheritance. | Check `logcat \| grep -i skyrm` for `kill backgroud app ... curAdj = 200`. | **v3.0 Local ADB Delegation**: Connect to `127.0.0.1:5555` from APK, send `exec:` command with `trap '' HUP INT TERM QUIT` & double-fork. Spawns Mihomo under `UID 2000 (shell)` and reparents to `PID 1 (init)`. Completely immune to OEM killers. |
| **SmartTube Launches Before Proxy Initialization Completes ("Network Error" on First Video Click)** | When launcher starts proxy and SmartTube simultaneously, SmartTube issues HTTP requests before Mihomo completes DNS and node handshakes (~300ms). | User sees blank feeds or "Network Error" on initial cold launch. | **Proxy Readiness Polling**: In Launcher `MainActivity`, actively poll `127.0.0.1:7890` (every 300ms, up to 3.5s). Once port is confirmed UP, display confirmation Toast and smoothly launch SmartTube. |
