...
| **OEM Background App Killers (Skyworth `SkyRMService`) Kill VPN / Accessibility Apps** | OEM proprietary daemons scan and terminate Java/ART background applications and accessibility services (`io.github.sds100.keymapper`). | Check `logcat \| grep -i skyrm` for `kill backgroud app ... curAdj = 200`. | Deploy native ELF Linux binary (`/data/local/tmp/mihomo`). Linux daemons launched via shell decouple from ART runtime and are 100% immune to OEM application killers. |
